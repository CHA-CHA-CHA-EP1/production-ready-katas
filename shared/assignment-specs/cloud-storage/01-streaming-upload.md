...
เขียนฟังก์ชัน `StreamUpload(client *minio.Client, bucket, key string, r io.Reader, size int64) error` ที่:

1. รับ `io.Reader` เป็น source ของ bytes — ห้าม buffer ทั้งหมดเข้า memory
...
- ห้ามเรียก `io.ReadAll`, `io.ReadFull`, หรือ read bytes ทั้งหมดเข้า `[]byte` ก่อน upload
- ต้องส่ง `io.Reader` โดยตรงไปให้ MinIO SDK

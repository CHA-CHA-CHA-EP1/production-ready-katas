...
**Language-specific requirements (Go):**
- **Go:** ห้ามใช้ `os.ReadFile` — ต้องเปิด/อ่าน/ปิดเอง เพื่อให้เห็น lifecycle ของ fd
- **Go:** `defer f.Close()` ต้องอยู่หลัง error check ของ open — ไม่ใช่ก่อน
...
implement `readConfig` ที่:
- **Input:** file path (string)
- **Output:** file content (bytes) หรือ error พร้อม context

1. รับ path ของไฟล์ config (plaintext, ขนาดไม่เกิน 1 MB)
2. คืน content ทั้งหมด
...
- ห้ามใช้ stdlib one-liner shortcut — ต้องเปิด/อ่าน/ปิด resource เอง เพื่อให้เห็น lifecycle
...
- [ ] คืน error ถ้าไฟล์ไม่มีอยู่ (file-not-found error check เป็น true)
...
- [ ] ไม่มี fd leak — หลังเรียกฟังก์ชัน ไม่ว่าจะ success หรือ error fd ต้องถูกปิดแล้ว (- [ ] ฟังก์ชันทนต่อการเรียกพร้อมกัน 100 ครั้ง (concurrent-safe) — ไม่มี shared mutable state

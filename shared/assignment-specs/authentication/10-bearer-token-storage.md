...
1. `issueTokenCookie(w, token)` — set httponly cookie สำหรับ web client พร้อม CSRF token แยกต่างหาก
2. `extractTokenFromRequest(r)` — ดึง token จาก request โดยรองรับทั้ง httponly cookie (web) และ `Authorization: Bearer` header (API client)

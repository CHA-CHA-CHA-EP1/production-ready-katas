...
```go
func TransformOnGet(ctx context.Context, event events.S3ObjectLambdaEvent) error
```
...
func addWatermark(src io.Reader, text string) (io.Reader, string, error)
// return: watermarked image reader, content-type, error

// fetchOriginal fetch image จาก presigned URL
func fetchOriginal(ctx context.Context, presignedURL string) (io.ReadCloser, error)
```

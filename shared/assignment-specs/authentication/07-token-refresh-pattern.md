...
type TokenStore interface {
    SaveRefreshToken(ctx cancellation context, userID, hashedToken string, expiry timestamp) error
    FindAndDeleteRefreshToken(ctx cancellation context, hashedToken string) (userID string, err error)
}

issueTokenPair(userID, secret) → accessToken, refreshToken , err error
refreshAccessToken(ctx, store, refreshToken, secret) → newAccessToken, newRefreshToken , err error
```
